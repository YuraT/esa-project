...
export function MitigationTableProvider({ children }: { children: React.ReactNode }) {
  ...
  // Table 2
  const [appParams, setAppParams] = useState<number[]>([0, 0, 0, 0]);
  const [inField, setInField] = useState<number[]>([0, 0, 0, 0, 0, 0, 0, 0, 0]);
  const [fieldAdjacent, setFieldAdjacent] = useState<number[]>([0, 0, 0, 0, 0, 0, 0]);
  const [systems, setSystems] = useState<number[]>([0, 0, 0]);
