      ...
      try {
        ...
        map.on("click", async (e) => {
          ...
          try {
            ...
            const [centerElev, eastElev, westElev, northElev, southElev] = elevations;

            // Calculate slope using the maximum gradient method
            const distance = offset * 111320; // Convert degrees to meters (approximate)
            const eastWestSlope = Math.abs(eastElev - westElev) / (2 * distance);
            const northSouthSlope = Math.abs(northElev - southElev) / (2 * distance);
            ...
            if (slopePercent <= 3) {
              ...
              slopeDescription = "This area has a gentle slope with low runoff potential.";
              suitabilityMessage = "✅ Suitable for reduced setback requirements.";
            } else if (slopePercent <= 8) {
              slopeCategory = "3-8% (Moderate Slope)";
              slopeColor = "#ffc107"; // yellow
              slopeDescription = "This area has a moderate slope with increased runoff potential.";
              ...
              slopeDescription = "This area has a steep slope with high runoff potential.";
              suitabilityMessage = "⚠️ Increased risk - standard setbacks required.";
            } else {
              slopeCategory = "> 15% (Very Steep)";
              slopeColor = "#dc3545"; // red
              slopeDescription = "This area has a very steep slope with very high runoff potential.";
              suitabilityMessage = "❌ High risk - consider additional mitigation measures.";
            }
    ...
    <div className={className}>
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-600 mb-2">
          This map helps you determine the slope percentage of your field using elevation data.
          Fields with slopes ≤ 3% may qualify for reduced setback requirements.{" "}
          <strong>
      ...
      <div className="mt-3 text-sm text-gray-600 space-y-1">
        ...
        <div className="mt-3 p-2 bg-blue-50 border border-blue-200 rounded">
          <p className="text-xs text-blue-800">
            <strong>💡 How to Use:</strong> Click anywhere on the map to calculate the slope percentage at that location.
            The calculation uses elevation data from surrounding points to determine the gradient. Green areas (≤ 3%)
            may qualify for reduced setback requirements due to lower runoff potential.
          </p>
        </div>

        <div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded">
          <p className="text-xs text-yellow-800">
            <strong>Note:</strong> Slope calculations are estimates based on elevation data.
            For critical applications, consider professional surveying. Results may vary based on
            data resolution and local terrain features.
          </p>
        </div>
        ...
        {!process.env.NEXT_PUBLIC_ARCGIS_API_KEY && (
          <div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded">
            <p className="text-xs text-yellow-800">
              <strong>API Key Required:</strong> Slope analysis requires an ArcGIS API key. Add
